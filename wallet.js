    ...
    .then(({ ask, totalAsk, bid, totalBid }) => {
        ...
        if (nombreCompletoLocalStorage) {
            ...
            wallet.forEach((movimiento) => {
                ...
                tdMovimiento.innerHTML = `$ ${movimiento.movimiento.toFixed(2)}`;
        ...
        inputSaldoInicial.addEventListener("keydown", function (event) {
            if (event.key === "Enter") {
                ...
                if (isNaN(saldoInicial) || saldoInicial <= 0) {
                    ...
                    swal(`Tu saldo inicial es de: ${saldoInicial.toFixed(2)}`);
                    setTimeout(() => {
                        location.reload();
                    }, 1000);
                }
...
function registrarTransaccion(monto) {
    ...
    swal("Tu movimiento fue aplicado por $" + monto.toFixed(2));
...
function restarSaldo() {
    ...
    egresosModal.addEventListener("submit", function (event) {
        ...
        if (!isNaN(egresos) && egresos > 0 && egresos <= saldo && saldo > 0 && movimientoReferencia.length >= 1) {
            ...
            swal(`Ingresa una cantidad válida, recuerda que tu saldo es de: $${saldo.toFixed(2)}`);
        }
    });
}
